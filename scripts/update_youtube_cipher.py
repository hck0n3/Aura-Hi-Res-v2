...
def get_player_url():
    ...
    req = urllib.request.Request(
        "https://music.youtube.com/", headers={"User-Agent": "Mozilla/5.0"}
    )
    html = urllib.request.urlopen(req).read().decode("utf-8")
    match = re.search(
        r'jsUrl["\']\s*:\s*["\']([^"\']+/player/[a-f0-9]{8}/[^"\']+)["\']', html
    )
    if not match:
...
def update_config():
    ...
    if not isinstance(configs, list):
        raise Exception(
            "player_configs.json is not a JSON array — refusing to guess a shape"
        )
    ...
    n_match = re.search(
        r'\.get\("n"\)\)\s*&&\s*\([a-zA-Z0-9$]+\s*=\s*new\s+g\.([a-zA-Z0-9$]+)',
        player_js,
    )
    if not n_match:
    ...
    sig_match = re.search(
        r"&&\s*\([a-zA-Z0-9$]+\s*=\s*([a-zA-Z0-9$]+)\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*[a-zA-Z0-9$]+\s*\)",
        player_js,
    )
    if sig_match:
        ...
        sig_match2 = re.search(
            r"&&\s*\([a-zA-Z0-9$]+\s*=\s*([a-zA-Z0-9$]+)\s*\(\s*(\d+)\s*,\s*decodeURIComponent",
            player_js,
        )
        if sig_match2:
            ...
            print(
                "Could not find sig expression, setting up empty structure for manual review"
            )

...
for name in [
    ...
    if "pin_to_speed_dial" in t:
        ...
        if "speedDialDao" not in t and "SpeedDialItem" not in t:
            ...
            if "speedDialDao" not in t2 and "SpeedDialItem" not in t2.replace(
                "import", ""
            ):
                # only remove import if no remaining SpeedDialItem usage
                if (
                    "SpeedDialItem" not in t2.split("import")[-1]
                    if False
                    else "SpeedDialItem(" not in t2 and "SpeedDialItem." not in t2
                ):
                    t2 = re.sub(
                        r"\nimport iad1tya\.echo\.music\.db\.entities\.SpeedDialItem\r?",
                        "",
                        t2,
                    )
            t = t2

...
5. youtubedl-android 0.17.3 → **0.18.1** — verde (build 13s). Hallazgo: las entradas del catálogo (`youtubedl-android-*`) **no las referencia ningún módulo** (ni build.gradle.kts ni código): es remanente del fork SimpMusic, sin efecto runtime. El test de descargas en dispositivo queda N/A para este bump.

...
/**
 * Whether [InnerTube.withRetry] should treat [error] as transient. Socket-level IO errors
 * and HTTP 5xx (ktor maps them to [ServerResponseException] under `expectSuccess = true`)
 * are transient; 4xx ([ClientRequestException]) and everything else — including
 * cancellation — are final and rethrown at once.
 *
 * HALLAZGO-028: right after Google sign-in, `account_menu` answers with a transient 500
 * `backendError` while the session propagates on YouTube's side; the old IOException-only
 * retry surfaced it as the owner's "press Accede again, then go back" login ritual.
 */
fun retryableInnerTubeError(error: Throwable): Boolean =
    error is IOException || error is ServerResponseException
...
class InnerTube {
    ...
    /**
     * Simple retry wrapper for transient errors (socket aborts, timeouts, HTTP 5xx).
     * Retries the given block up to [maxAttempts] times with exponential backoff.
     * Cancellation is respected: [retryableInnerTubeError] is false for
     * [kotlinx.coroutines.CancellationException], so it is rethrown at once.
     */
    private suspend fun <T> withRetry(
        ...
        while (true) {
            try {
                ...
                if (!retryableInnerTubeError(e)) throw e
                attempt++
    ...
    // HALLAZGO-028: the post-login 500 backendError clears within ~1-2 s as the Google
    // session propagates, so this one endpoint retries on a slower clock than the default.
    suspend fun accountMenu(client: YouTubeClient) = withRetry(initialDelay = 1000L) {

...
object NewPipeExtractor {
    ...
    // HALLAZGO-042: skips PipePipe extraction while it is failing consecutively (anti-bot wall
    ...
    fun newPipePlayer(videoId: String): List<Pair<Int, String>> {
        init()
        // HALLAZGO-042: on the owner's device/IP PipePipe's anonymous extraction hits YouTube's
        // anti-bot wall EVERY time (~1.7 s wasted) before falling back to BraveNewPipe. Once the
        // breaker has seen enough consecutive failures it skips PipePipe for a cooldown and the
        // resolve goes straight to the fallback that is known to work. A success closes it.
        var pipePipeStreams = emptyList<Pair<Int, String>>()
        if (pipePipeBreaker.shouldSkip(System.currentTimeMillis())) {
            timber.log.Timber.tag("RESOLVE_CIPHER").d(
            ...
            pipePipeBreaker.recordResult(pipePipeStreams.isNotEmpty(), System.currentTimeMillis())
        }

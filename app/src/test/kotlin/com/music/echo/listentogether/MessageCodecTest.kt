...
/**
 * Ported from SimpMusic's MessageCodecTest (which ported it from Metrolist, GPL-3.0).
 *
 * These are conformance tests, not unit tests: they exist to catch the day this codec stops
 * producing the bytes Metrolist's servers and clients expect. `envelopeFieldNumbersMatchTheProtoSchema`
...
class MessageCodecTest {
    ...
    fun chatFromAMetrolistPeerDoesNotBreakTheStream() {
        // The chat message exists in the protocol (a Metrolist user in the room WILL send it) but
        // this client draws no chat UI — it must decode to null and never kill the frame loop.

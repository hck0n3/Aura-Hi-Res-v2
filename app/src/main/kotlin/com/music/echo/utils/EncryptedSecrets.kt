...
object SecretCrypto {
    ...
    // HALLAZGO-062: la SecretKey del AndroidKeyStore es un handle al material que vive en el
    // hardware (nunca expone el material en sí), así que cachearla en memoria es seguro y evita
    // pagar `KeyStore.load(null)` + `getEntry()` — una llamada binder/TEE de ~ms — en CADA
    // cifrado/descifrado. Con ~560 sitios de lectura y descifrado por colector y por emisión,
    // ese costo repetido era el que bloqueaba el hilo principal (reacción lenta, animaciones
    // trabosas). Una sola ida al Keystore por proceso.
    @Volatile
    private var cachedKey: SecretKey? = null

    private val keyLookupCount = AtomicInteger(0)
    ...
    fun decryptOrNull(stored: String): String? {
        ...
        // HALLAZGO-062: sin Keystore disponible no hay con qué descifrar; null directo en vez de
        // reintentar el Keystore en cada operación (cada intento fallido era otra llamada binder
        // con su excepción, pagada por colector y por emisión). `keystoreReady` ya memoiza la
        // disponibilidad para el proceso entero, igual que hace `encrypt`.
        if (!keystoreReady) return null
        return try {
    ...
    /** Solo tests: cuántas veces se fue al Keystore por la clave (debe ser ≤ 1 por proceso). */
    internal fun keyLookupCountForTest(): Int = keyLookupCount.get()
}
...
class EncryptedSecretsDataStore(
    ...
    private class DecryptedView(val prefs: Preferences, val locked: Set<String>)

    // HALLAZGO-062: el DataStore emite la MISMA instancia de Preferences a todos sus colectores,
    // pero el `.map` de descifrado corría por colector y por emisión: N colectores (decenas de
    // `rememberPreference`/`collectAsState` vivos) × descifrado completo, en el hilo principal.
    // La vista descifrada es función pura de la instancia guardada, así que se memoiza por
    // identidad: el primer colector de cada emisión paga UNA pasada de descifrado y el resto
    // reutiliza la vista. Peor caso bajo contención: dos pasadas simultáneas (resultado idéntico).
    private val viewCache = AtomicReference<Pair<Preferences, DecryptedView>?>(null)

    private val viewComputationCount = AtomicInteger(0)

    private fun decryptedViewFor(stored: Preferences): DecryptedView {
        viewCache.get()?.let { (input, view) -> if (input === stored) return view }
        viewComputationCount.incrementAndGet()
        val locked = mutableSetOf<String>()
        val prefs = stored.decryptedView(locked)
        val view = DecryptedView(prefs, locked.toSet())
        viewCache.set(stored to view)
        return view
    }

    override val data: Flow<Preferences> = delegate.data.map { decryptedViewFor(it).prefs }

    override suspend fun updateData(transform: suspend (t: Preferences) -> Preferences): Preferences {
        val updated =
            delegate.updateData { stored ->
                val view = decryptedViewFor(stored)
                val transformed = transform(view.prefs)
                transformed.reencrypted(stored, view.locked, view.prefs)
            }
        return decryptedViewFor(updated).prefs
    }
     ...
     * algo que no pudimos leer. Y un valor que el transform NO cambió también conserva su
     * ciphertext original (HALLAZGO-062): re-cifrarlo en cada escritura ajena costaba otra pasada
     * de Keystore por clave sin aportar nada — GCM ya usa IV aleatorio por cifrado.
     */
    private fun Preferences.reencrypted(
        ...
        for (key in SENSITIVE_KEYS) {
            ...
            val toStore =
                when {
                    key.name in locked && newValue.isEmpty() && storedValue != null -> storedValue
                    storedValue != null && newValue == view[key] -> storedValue
                    else -> SecretCrypto.encrypt(newValue)
                }
            mutable[key] = toStore
        }
        return mutable.toPreferences()
    ...
    /** Solo tests: cuántas veces se computó la vista descifrada (una por emisión, no por colector). */
    internal fun viewComputationCountForTest(): Int = viewComputationCount.get()
}

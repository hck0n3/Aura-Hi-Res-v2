 ...
 *
 * Android 12+ mutes AudioRecord for cached (backgrounded) apps: without this service, backgrounding
 * or locking the screen during the ~10s in-app listen makes the mic deliver silence and recognition
 * always fails. Running the capture as a foregroundServiceType="microphone" service keeps the mic
 * alive, shows live progress in a notification, and persists the result notification when done.
 */
class RecognitionForegroundService : Service() {
    ...
    private fun handleSuccess(result: RecognitionResult) {
        ...
        updateNotification(
            ...
            largeIcon = null,
            actionIntent = pendingIntent,
            actionTitle = getString(R.string.listen_on_aura),
        )

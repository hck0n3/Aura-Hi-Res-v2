...
import iad1tya.echo.music.utils.shouldRecoverLibrarySyncOnStart
import iad1tya.echo.music.utils.shouldSyncOnCookieChange
...
/**
 * HALLAZGO-061: process-scoped coroutine access from any Context. The login completion must run
 * here (NOT in the LoginScreen's composition scope) so it survives the user leaving the screen
 * during the silent validation window — a composition-scoped launch was cancelled on disposal
 * and the library syncs never got enqueued.
 */
val Context.applicationScope: CoroutineScope
    get() = (applicationContext as App).applicationScope
...
class App : Application(), SingletonImageLoader.Factory, androidx.work.Configuration.Provider {
    ...
    override fun onCreate() {
        ...
        // HALLAZGO-053: Android 14+ refuses Visualizer init (ERROR_NO_INIT, -3) without
        // RECORD_AUDIO. The hub gates its attach on the live permission state; wire that state
        // here so the EQ meter can show the "grant microphone" hint instead of a throw storm.
        iad1tya.echo.music.ui.newui.AuraVisualizerHub.recordAudioGrantedProvider = {
        ...
        // HALLAZGO-060 (diferido de la fila 178) + auditoría de latencia de entrada: un ÚNICO colector
        // de proceso publica cada emisión del DataStore de ajustes en PrefsBridge, para que las ~640
        // semillas de rememberPreference y las lecturas calientes del servicio lean sin runBlocking.
        // Primero de la zona de proceso por defecto a propósito: cuanto antes llegue la primera emisión
    ...
    private suspend fun initializeSettings() {
        ...
        // with non-DataStore side effects stay SEPARATE: migrateAudioDefaultsV2 + migrateEqPreampDefaultV3
        // (EQ repo + echo_eq_prefs, two-phase on seed success / default bump) keep their position BETWEEN
        // the batches so Safe Volume's forced-ON still lands AFTER them,
        ...
        // SEPARATE (same EQ side effects): bumps the seeded 0.0 preamp to the +3 dB default (owner
        // request, HALLAZGO-046). MUST run after migrateAudioDefaultsV2 so the seed exists first.
        migrateEqPreampDefaultV3(settings)
        // SEPARATE (same EQ side effects): lowers the +3 dB preamp default to the owner's final
        ...
        // never written = the login completion got cancelled before it enqueued the library syncs
        // (the exact state the owner's v2.0.11 log showed). Heal it once at cold start; after the
        // first completed sync the stamp is > 0 and this is a no-op forever.
        if (shouldRecoverLibrarySyncOnStart(isLoggedCookie(settings[InnerTubeCookieKey]), syncUtils.lastLikedSyncTimeMs())) {
            Timber.d("Cold start: logged-in account never synced its library, enqueueing recovery syncs (HALLAZGO-061)")
    ...
    /**
     * One-time (V3, HALLAZGO-046): raise the EQ preamp default from 0.0 to +3.0 dB (owner request) for
     * everyone still on the seeded default. Runs AFTER [migrateAudioDefaultsV2] so the seed it bumps
     * already exists on fresh installs (both writers agree on +3 — ordering cannot undo it). Users who
     * chose their own preamp (anything != 0.0) keep it. Writes BOTH the DSP source of truth (the profile
     * repo — [iad1tya.echo.music.playback.MusicService.currentEqPreampDb] prefers the profile over the
     * mirror) AND the `echo_eq_prefs` mirror the EQ screen reads, mirroring migrateAudioDefaultsV2.
     * Two-phase: the flag is only stamped when the write succeeded, so a failure retries next launch.
     * While NO EQ state exists yet (the V2 seed has not landed) it also retries instead of stamping,
     * so it can never race the seed into a half-applied EQ.
     */
    private suspend fun migrateEqPreampDefaultV3(settings: androidx.datastore.preferences.core.Preferences) {
        if (settings[iad1tya.echo.music.constants.EqPreampDefault3DbAppliedKey] == true) return
        val applied = runCatching {
            ...
            if (!(storedPreamp.isNaN() || storedPreamp == 0.0f)) return@runCatching true
            // Repo first: the effective profile (unsaved ?: active) wins over the prefs mirror at runtime.
            val effective = eqRepo.unsavedProfile.value ?: eqRepo.activeProfile.value
            if (effective != null && effective.preamp == 0.0) {
                val raised = effective.copy(preamp = 3.0)
                eqRepo.saveProfile(raised)
                eqRepo.setUnsavedProfile(raised)
                eqRepo.setActiveProfile(raised.id)
            }
            eqPrefs.edit().putFloat("preampDb", 3.0f).apply()
            true
        }.onFailure { reportException(it) }.getOrDefault(false)
        if (applied) {
            dataStore.edit { it[iad1tya.echo.music.constants.EqPreampDefault3DbAppliedKey] = true }
        }
    }

    /**
     * One-time (V4, owner request 2026-08-26): lower the EQ preamp default from +3.0 to +2.0 dB — the
     * owner tested +3 and wants the stock experience at +2. Runs AFTER [migrateEqPreampDefaultV3] so
     * an install V3 just raised to +3 settles on +2 in the same launch (both writers agree on +2 —
     * ordering cannot undo it). "Exactly +3.0" is treated as "still on the old default", the same
     * convention V3 used for 0.0; users who chose any other preamp keep it. Writes BOTH the DSP
     * source of truth (the profile repo — MusicService.currentEqPreampDb prefers the profile over the
     * mirror) AND the `echo_eq_prefs` mirror the EQ screen reads. Two-phase like V3: the flag is only
     * stamped on success, and while NO EQ state exists yet it retries instead of stamping.
     */
    private suspend fun migrateEqPreampDefaultV4(settings: androidx.datastore.preferences.core.Preferences) {
        if (settings[iad1tya.echo.music.constants.EqPreampDefault2DbAppliedKey] == true) return
        val applied = runCatching {
            val eqPrefs = applicationContext.getSharedPreferences("echo_eq_prefs", Context.MODE_PRIVATE)
            val eqRepo = eqProfileRepository.get()
            val hasAnyEqState = eqPrefs.contains("enabled") || eqPrefs.contains("preampDb") ||
            ...
            if (!hasAnyEqState) return@runCatching false
            val storedPreamp = eqPrefs.getFloat("preampDb", Float.NaN)
            if (!(storedPreamp.isNaN() || storedPreamp == 3.0f)) return@runCatching true
            val effective = eqRepo.unsavedProfile.value ?: eqRepo.activeProfile.value
            if (effective != null && effective.preamp == 3.0) {
                val lowered = effective.copy(preamp = 2.0)
                eqRepo.saveProfile(lowered)
                eqRepo.setUnsavedProfile(lowered)
                eqRepo.setActiveProfile(lowered.id)
            }
            eqPrefs.edit().putFloat("preampDb", 2.0f).apply()
            true
        }.onFailure { reportException(it) }.getOrDefault(false)
        if (applied) {
    ...
    private fun observeSettingsChanges() {
        ...
        applicationScope.launch(Dispatchers.IO) {
            // HALLAZGO-061: the cookie persisting is NOT the same as the login completing — the
            ...
            dataStore.data
                ...
                .collect { cookie ->
                    ...
                    if (!firstEmission && shouldSyncOnCookieChange(previousCookie, cookie)) {
                        Timber.d("Cookie watcher: new logged-in session detected, enqueueing library syncs (HALLAZGO-061)")
                        syncUtils.syncLibraryAfterLogin()
                    }
                    firstEmission = false

...
private fun AuraPlayerShape(
    ...
    val rhythmLevel by rememberAuraRhythmLevel(
        ...
        // HALLAZGO-027 (second pass): captureEnabled must stay STABLE across sheet flips — the
        // Visualizer is no longer built/torn down per gesture; [active] gates only the per-frame
        // smoothing loop inside.
        captureEnabled = !highPerfMode && !isCasting,
    ...
    // animated ground by song rhythm across the player). Quantized via derivedStateOf
    // (HALLAZGO-027, second pass): the raw level moves every frame while playing, and reading it
    // here in composition recomposed the whole player tree per frame. Now composition only runs
    // when the level crosses a quantization bucket.
    val rhythmIntensity by remember { derivedStateOf { quantizeAuraRhythmIntensity(rhythmLevel) } }
                    ...
                    if (isFullScreen && enableLyricsThumbnailPlayPause) {
                        ...
                        AuraArtwork(
                            ...
                            AsyncImage(
                                ...
                                // that ignored the switch. HALLAZGO-057: VIDEO covers always fill
                                // the frame regardless of the preference.
                                contentScale = if (cropAlbumArt || mediaMetadata?.isVideoSong == true) ContentScale.Crop else ContentScale.Fit,
                        ...
                        Box(
                            ...
                            if (!hasFirstVideoFrame) {
                                AsyncImage(
                                    model = mediaMetadata?.thumbnailUrl,
                                    contentDescription = null,
                                    // HALLAZGO-057: this overlay covers the VIDEO surface — the
                                    // video's own thumbnail always fills the slot (Crop).
                                    contentScale = if (cropAlbumArt || mediaMetadata?.isVideoSong == true) ContentScale.Crop else ContentScale.Fit,
                                    modifier = Modifier.fillMaxSize(),
                                )
                            }
 ...
 * [AuraPalette.OnGround] over its own 45 % scrim rather than the sheet's 48–55 % steps. Three rules
 ...
 *  · **Motion is dropped.** The pill is always on screen — on every tab, during every scroll,
 *    behind the expanded player. Its ground must never own a per-frame writer; `drift` and `spin`
 *    are the sheet's, not the pill's. (HALLAZGO-059.)
...
internal fun auraPillRecipe(recipe: AuraGroundRecipe): AuraGroundRecipe = recipe.copy(
    ...
    // HALLAZGO-059 guardian: the pill is on screen on every tab, during every scroll, behind the
    // expanded player — drift/spin here would invalidate its draw every frame, forever. Motion is
    // the sheet's (it only lives while expanded); the pill draws the same layers still.
    drift = false,
    spin = false,
)
 ...
 *
 * HALLAZGO-060: the lobes used to be shaded as two GIANT radial-gradient fills on every frame
 * (radius = max(w,h)×0.72 → a ~20 MP gradient evaluation per lobe on a QHD+ panel), the fill-rate
 * cost of the GLOW_ANIMATED style. Same remedy as the bloom (HALLAZGO-055): each lobe is
 * rasterized ONCE per artwork into a capped bitmap ([AuraLobeRaster], remembered here so it
 * survives recomposition) and every frame after that is two texture blits positioned by the
 * drift phase — read inside `onDrawBehind`, so drifting invalidates DRAW only and allocates
 * nothing per frame.
 */
@Composable
private fun AuraLobeGround(colors: AuraBloomColors, alpha: Float, drift: Boolean) {
    val raster = remember { AuraLobeRaster() }
    if (drift) {
        ...
        Box(Modifier.fillMaxSize().auraLobes(raster, colors, alpha) { phase })
    } else {
        Box(Modifier.fillMaxSize().auraLobes(raster, colors, alpha) { 0f })
    }
}

/**
 * HALLAZGO-060: per-composition raster cache for the two lobe bitmaps, keyed by artwork colours.
 * The drawWithCache block re-runs on every recomposition of the ground layer (rhythm bucket
 * crossings, several per second while music plays), so the bitmaps must live OUTSIDE it — the
 * same lesson the bloom raster learned. Draw phase only (single UI thread), no synchronization.
 */
private class AuraLobeRaster {
    private var keyColors: AuraBloomColors? = null
    private var first: ImageBitmap? = null
    private var second: ImageBitmap? = null

    fun imagesFor(colors: AuraBloomColors): Pair<ImageBitmap, ImageBitmap> {
        val f = first
        val s = second
        if (f != null && s != null && keyColors == colors) return f to s
        val newFirst = rasterizeAuraLobe(colors.topLeft)
        val newSecond = rasterizeAuraLobe(colors.topRight)
        keyColors = colors
        first = newFirst
        second = newSecond
...
/**
 * HALLAZGO-060: diameter of one lobe raster, in px. The lobe is a single soft radial gradient —
 * as low-frequency as they come — so the same cap the bloom uses costs no visible fidelity and
 * keeps each bitmap around ~4 MB even when the blit upscales it to a QHD+ sheet.
 */
private const val AURA_LOBE_RASTER_DIM = 1080

/** Renders one full-strength lobe (radial gradient, colour → transparent) into a square bitmap. */
private fun rasterizeAuraLobe(color: Color): ImageBitmap {
    val dim = AURA_LOBE_RASTER_DIM
    val radius = dim / 2f
    ...
    CanvasDrawScope().draw(
        ...
        drawRect(
            ...
            topLeft = Offset.Zero,
            size = Size(dim.toFloat(), dim.toFloat()),
        )
    }
    return image
}
...
private fun Modifier.auraLobes(
    ...
    // Cache lookup (rasterizes only on an artwork change); alpha is applied at draw time below,
    // so an intensity change can never invalidate the bitmaps.
    val (first, second) = raster.imagesFor(colors)
    val radius = maxOf(w, h) * 0.72f
    val dstDim = (radius * 2f).toInt()
    val dst = IntSize(dstDim, dstDim)
    ...
    onDrawBehind {
        ...
        // fully leave the sheet. sin/cos of one float: the whole per-frame cost besides the two blits.
        ...
        translate(x1 - radius, y1 - radius) {
            drawImage(first, dstSize = dst, alpha = a)
        }
        translate(x2 - radius, y2 - radius) {
            drawImage(second, dstSize = dst, alpha = a * 0.85f)
        }
    }
}

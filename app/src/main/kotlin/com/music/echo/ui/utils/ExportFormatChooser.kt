...
fun ExportFormatChooserDialog(
    ...
    LaunchedEffect(songId, hasMusicVideo) {
        ...
        val result = withContext(Dispatchers.IO) {
            // HALLAZGO-035: probe the SAME chain the export itself uses (NewPipe ANDROID_VR first,
            // burned TVHTML5 diag only as fallback) — probing diag alone left the row dead while
            // video mode played fine. Two chained resolves get a longer budget than the old single one.
            withTimeoutOrNull(30_000L) {
                val cm = context.getSystemService<ConnectivityManager>()
                    ?: return@withTimeoutOrNull VideoProbe.Unavailable(R.string.export_video_unavailable)
                val resolved = YTPlayerUtils.videoStreamUrlForExport(
                    videoId = songId,

...
fun AutoEqScreen(navController: NavController) {
    ...
    // HALLAZGO-049: Auto-EQ also wears the artwork bloom while the Aura skin is on, so the screen
    // follows the cover live like the rest of the redesign. Classic surface stays as the fallback.
    ...
    Column(
        Modifier
            .fillMaxSize()
            .then(if (bloomGround) Modifier.auraScreenBackground(bloom, intensity = 0.40f) else Modifier)
